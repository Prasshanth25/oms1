...
public final class SystemClock implements TimeSource<Moment> {
    ...
    public long currentTimeInMillis() {
        if (this.monotonic || MONOTON_MODE) {
            long utcNanos = utcNanos();
            return MathUtils.safeMultiply(LeapSeconds.getInstance().strip(MathUtils.floorDivide(utcNanos, 1000000000)), 1000L) + MathUtils.floorModulo(utcNanos, 1000000);
        }
    ...
    public long currentTimeInMicros() {
        if (this.monotonic || MONOTON_MODE) {
            long utcNanos = utcNanos();
            return MathUtils.safeMultiply(LeapSeconds.getInstance().strip(MathUtils.floorDivide(utcNanos, 1000000000)), 1000000L) + MathUtils.floorModulo(utcNanos, 1000);
        }
    ...
    public long realTimeInMicros() {
        ...
        long currentTimeMillis = System.currentTimeMillis();
        return MathUtils.safeMultiply(LeapSeconds.getInstance().enhance(MathUtils.floorDivide(currentTimeMillis, 1000)), 1000000L) + (MathUtils.floorModulo(currentTimeMillis, 1000) * 1000);
    }
    ...
    public SystemClock synchronizedWith(TimeSource<?> timeSource) {
        ...
        return new SystemClock(this.monotonic, MathUtils.safeSubtract(MathUtils.safeMultiply(from.getElapsedTime(TimeScale.UTC), 1000000000L) + from.getNanosecond(TimeScale.UTC), MONOTON_MODE ? System.nanoTime() : PROVIDER.getNanos()));
    }
