...
public final class SPX implements Externalizable {
    ...
    public static void writeTransitions(ZonalTransition[] zonalTransitionArr, int i, DataOutput dataOutput) throws IOException {
    ...
    private static List<ZonalTransition> readTransitions(ObjectInput objectInput) throws IOException {
        ...
        while (i3 < readInt) {
            ...
            if (timeOfDayT == -1) {
                ...
                readByte = ((((((((readByte2 & 3) << 16) | ((objectInput.readByte() & UByte.MAX_VALUE) << 8)) | (objectInput.readByte() & UByte.MAX_VALUE)) * 86400) + POSIX_TIME_1825) + timeOfDayT) - 7200) - readOffset;
            }
            if (readByte <= j) {
    ...
    private static Object readRuleBasedTransitionModel(ObjectInput objectInput) throws IOException, ClassNotFoundException {
        long readByte;
        int readByte2 = objectInput.readByte() & UByte.MAX_VALUE;
        if (readByte2 == 255) {
            readByte = objectInput.readLong();
        } else {
            readByte = (((readByte2 << 16) + ((objectInput.readByte() & UByte.MAX_VALUE) << 8) + (255 & objectInput.readByte())) * 900) + POSIX_TIME_1825;
        }
