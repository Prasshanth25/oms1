...
public abstract class Timezone implements Serializable {
    ...
    public static List<TZID> getAvailableIDs(String str) {
        if (!str.equals("INCLUDE_ALIAS")) {
            ...
            for (String str2 : provider.getAvailableIDs()) {
    ...
    public static Set<TZID> getPreferredIDs(Locale locale, boolean z, String str) {
        ...
        for (String str2 : specificZoneNameRepository.getPreferredIDs(locale, z)) {
            hashSet.add(resolve(str2));
        }
    ...
    public static TZID normalize(String str) {
        ...
        if (!((str2.isEmpty() || str2.equals(NAME_DEFAULT)) ? true : true) && !str2.equals("WINDOWS") && !str2.equals("MILITARY") && (zoneModelProvider = PROVIDERS.get(str2)) == null) {
            String str4 = str2.equals(NAME_TZDB) ? "TZDB provider not available: " : "Timezone model provider not registered: ";
    ...
    public static String getProviderInfo() {
        ...
        for (String str : PROVIDERS.keySet()) {
            ZoneModelProvider zoneModelProvider = PROVIDERS.get(str);
    ...
    private static Timezone getTZ(TZID tzid, String str, boolean z) {
        ...
        ZoneModelProvider zoneModelProvider = DEFAULT_PROVIDER;
        z2 = (str3.isEmpty() || str3.equals(NAME_DEFAULT)) ? true : true;
    ...
    public static class ZonalKeys {
        ...
        ZonalKeys() {
            ...
            for (Map.Entry entry : Timezone.PROVIDERS.entrySet()) {
                ZoneModelProvider zoneModelProvider = (ZoneModelProvider) entry.getValue();
                if (zoneModelProvider != Timezone.PLATFORM_PROVIDER || Timezone.DEFAULT_PROVIDER == Timezone.PLATFORM_PROVIDER) {
                    for (String str : zoneModelProvider.getAvailableIDs()) {
                        TZID resolve = Timezone.resolve(str);
                        if (!arrayList.contains(resolve)) {
                            arrayList.add(resolve);
                        }
                    }
                    arrayList2.addAll(arrayList);
                    for (String str2 : zoneModelProvider.getAliases().keySet()) {
                        TZID resolve2 = Timezone.resolve(str2);
    ...
    private static class PlatformZoneProvider implements ZoneModelProvider, ZoneNameProvider {
        ...
        public String getDisplayName(String str, NameStyle nameStyle, Locale locale) {
            ...
            return findZone.getID().equals(str) ? findZone.getDisplayName(nameStyle.isDaylightSaving(), !nameStyle.isAbbreviation(), locale) : "";
        }
    }
}
