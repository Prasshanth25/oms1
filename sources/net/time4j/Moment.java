...
public final class Moment extends TimePoint<TimeUnit, Moment> implements UniversalTime, Temporal<UniversalTime> {
    ...
    static {
        ...
        TimeAxis.Builder up = TimeAxis.Builder.setUp(TimeUnit.class, Moment.class, new Merger(null), moment, moment2);
            ...
            if (leapSeconds.isEnabled()) {
                if (timeScale != TimeScale.UTC) {
                    if (timeScale == TimeScale.TAI) {
                        ...
                        long safeAdd3 = MathUtils.safeAdd(j3, (long) UTC_GPS_DELTA);
                    ...
                    } else if (timeScale != TimeScale.UT) {
                    ...
                    if (enhance != 0 || strip == MAX_LIMIT) {
                        ...
                        this.fraction = POSITIVE_LEAP_MASK | i2;
                    } else {
    ...
    public <C extends CalendarVariant<C>> GeneralTimestamp<C> toGeneralTimestamp(CalendarFamily<C> calendarFamily, String str, TZID tzid, StartOfDay startOfDay) {
        PlainTimestamp zonalTimestamp = toZonalTimestamp(tzid);
        return GeneralTimestamp.of(zonalTimestamp.minus(startOfDay.getDeviation(zonalTimestamp.getCalendarDate(), tzid), ClockUnit.SECONDS).getCalendarDate().transform((Class<CalendarVariant>) calendarFamily.getChronoType(), str), zonalTimestamp.getWallTime());
    }
    ...
    private double getModernUT() {
        ...
        return Double.compare(1.0E9d - ((elapsedTimeUTC - ((double) floor)) * 1.0E9d), 1.0d) < 0 ? floor + 1 : elapsedTimeUTC;
    }
    ...
    public static final class Operator implements ChronoOperator<Moment> {
        ...
        public Moment apply(Moment moment) {
            ...
            if (!moment.isLeapSecond() || !isNonIsoOffset(timezone, moment)) {
                ...
                PlainTimestamp plainTimestamp = (PlainTimestamp) moment.in(timezone).with((ChronoOperator<T>) this.delegate);
    ...
    private static class TimeUnitRule implements UnitRule<Moment> {
        ...
        public long between(Moment moment, Moment moment2) {
            ...
            if (this.unit.compareTo(TimeUnit.SECONDS) >= 0) {
                ...
                int i = (safeAdd > 0L ? 1 : (safeAdd == 0L ? 0 : -1));
                if (i < 0) {
    ...
    /* renamed from: net.time4j.Moment$1 */
    /* loaded from: classes2.dex */
