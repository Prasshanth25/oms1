...
public final class HebrewTime extends TimePoint<Unit, HebrewTime> implements Temporal<HebrewTime> {
    ...
    public static ChronoFunction<Moment, HebrewTime> at(final SolarTime solarTime) {
        return new ChronoFunction<Moment, HebrewTime>() { // from class: net.time4j.calendar.HebrewTime.1
            /* JADX WARN: Removed duplicated region for block: B:73:0x008f A[ADDED_TO_REGION] */
    ...
    public static ChronoFunction<Moment, HebrewTime> at(final TZID tzid) {
        return new ChronoFunction<Moment, HebrewTime>() { // from class: net.time4j.calendar.HebrewTime.2
            ...
            public HebrewTime apply(Moment moment) {
                ...
                return new HebrewTime((wallTime.getHour() + 6) % 24, ((BigDecimal) wallTime.get(PlainTime.DECIMAL_HOUR)).subtract(new BigDecimal(wallTime.getHour())).multiply(new BigDecimal((int) HebrewTime.PARTS_IN_HOUR)).intValue());
            }
        };
    }
    ...
    public Moment on(HebrewCalendar hebrewCalendar, SolarTime solarTime) {
        ...
        if (isNight()) {
            apply2 = solarTime.sunset().apply(plainDate.minus(CalendarDays.ONE));
            apply = solarTime.sunrise().apply(plainDate);
            minus = this;
        } else {
            apply = solarTime.sunset().apply(plainDate);
            minus = minus(12L, Unit.HOURS);
            apply2 = solarTime.sunrise().apply(plainDate);
        }
        if (apply2 == null || apply == null) {
            return null;
        }
        int until = (int) apply2.until(apply, (Moment) TimeUnit.SECONDS);
        if (apply2.getNanosecond() > apply.getNanosecond()) {
            until--;
        }
        return apply2.plus((long) Math.floor((minus.getTimeOfDay() * until) / 12960.0d), (long) TimeUnit.SECONDS);
    }

    public Moment on(HebrewCalendar hebrewCalendar, Timezone timezone) {
        return hebrewCalendar.at((PlainTime) PlainTime.of(18).with(PlainTime.DECIMAL_HOUR, (ZonalElement<BigDecimal>) new BigDecimal(this.part).setScale(15, RoundingMode.UNNECESSARY).divide(new BigDecimal((int) PARTS_IN_HOUR), RoundingMode.FLOOR).add(new BigDecimal((this.hour23 + 18) % 24)))).in(timezone, StartOfDay.EVENING);
    }
    ...
    public static class ClockUnitRule implements UnitRule<HebrewTime> {
        ...
        public HebrewTime addTo(HebrewTime hebrewTime, long j) {
            ...
            if (i3 == 1) {
                ...
                i = MathUtils.floorModulo(safeAdd, (int) HebrewTime.PARTS_IN_HOUR);
                i2 = MathUtils.floorModulo(MathUtils.safeAdd(hebrewTime.hour23, MathUtils.floorDivide(safeAdd, (int) HebrewTime.PARTS_IN_HOUR)), 24);
            } else {
    ...
    /* renamed from: net.time4j.calendar.HebrewTime$3 */
    /* loaded from: classes2.dex */
        ...
        public HebrewTime createFrom2(ChronoEntity<?> chronoEntity, AttributeQuery attributeQuery, boolean z, boolean z2) {
            ...
            if (chronoEntity.contains(HebrewTime.PART_OF_HOUR)) {
                ...
                if (i < 0 || i >= HebrewTime.PARTS_IN_HOUR) {
                    ...
                    chronoEntity.with((ChronoElement<ValidationElement>) validationElement, (ValidationElement) ("PART_OF_HOUR out of range: " + i));
                    return null;
                }
            } else {
                i = 0;
            ...
            if (chronoEntity.contains(HebrewTime.CLOCK_CYCLE) && chronoEntity.contains(HebrewTime.CLOCK_HOUR)) {
                ...
                if (i2 < 1 || i2 > 12) {
                    ...
                    chronoEntity.with((ChronoElement<ValidationElement>) validationElement2, (ValidationElement) ("CLOCK_HOUR out of range: " + i2));
                    return null;
                }
                ...
                if (i3 < 0 || i3 > 23) {
                    ...
                    chronoEntity.with((ChronoElement<ValidationElement>) validationElement3, (ValidationElement) ("DIGITAL_HOUR out of range: " + i3));
                    return null;
                }
