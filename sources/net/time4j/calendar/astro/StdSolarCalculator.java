...
public enum StdSolarCalculator implements SolarTime.Calculator {
    SIMPLE { // from class: net.time4j.calendar.astro.StdSolarCalculator.1
        ...
        private Moment event(CalendarDate calendarDate, double d, double d2, double d3, boolean z) {
            ...
            double d4 = d2 / 15.0d;
            double dayOfYear = gregorian.getDayOfYear() + (((z ? 6 : 18) - d4) / 24.0d);
            ...
            long longValue = (((Long) gregorian.get(EpochDays.UTC)).longValue() * 86400) + ((int) Math.floor((d5 - d4) * 3600.0d));
    ...
    NOAA { // from class: net.time4j.calendar.astro.StdSolarCalculator.2
        ...
        private Moment event(boolean z, CalendarDate calendarDate, double d, double d2, double d3) {
            ...
            return (Moment) fromLocalEvent.plus(floor, (long) TimeUnit.SECONDS).plus((int) ((localHourAngle2 - floor) * 1.0E9d), (long) TimeUnit.NANOSECONDS).with((ChronoElement<ChronoElement<TimeUnit>>) Moment.PRECISION, (ChronoElement<TimeUnit>) TimeUnit.SECONDS);
        }
        ...
        private Moment event(boolean z, CalendarDate calendarDate, double d, double d2, double d3) {
            ...
            while (true) {
                ...
                if (Math.abs(localHourAngle - d4) < 15.0d) {
                    ...
                    return (Moment) fromLocalEvent.plus(floor, (long) TimeUnit.SECONDS).plus((int) ((localHourAngle - floor) * 1.0E9d), (long) TimeUnit.NANOSECONDS).with((ChronoElement<ChronoElement<TimeUnit>>) Moment.PRECISION, (ChronoElement<TimeUnit>) TimeUnit.SECONDS);
                }
                d4 = localHourAngle;
            }
        }
    ...
    public static void nutations(double d, double[] dArr) {
