...
public final class SolarTime implements GeoLocation, Serializable {
    ...
    public static ChronoFunction<Moment, PlainTimestamp> apparentAt(final ZonalOffset zonalOffset) {
        return new ChronoFunction<Moment, PlainTimestamp>() { // from class: net.time4j.calendar.astro.SolarTime.12
            ...
            public PlainTimestamp apply(Moment moment) {
                PlainTimestamp onAverage = SolarTime.onAverage(moment, zonalOffset);
                double equationOfTime = SolarTime.equationOfTime(moment);
                long floor = (long) Math.floor(equationOfTime);
                return onAverage.plus(floor, ClockUnit.SECONDS).plus((int) ((equationOfTime - floor) * 1.0E9d), ClockUnit.NANOS);
            }
        };
    }
    ...
    public static Moment fromLocalEvent(CalendarDate calendarDate, int i, double d, String str) {
        ...
        double equationOfTime = calculator.equationOfTime(JulianDay.getValue(of, TimeScale.TT));
        long floor2 = (long) Math.floor(equationOfTime);
        double equationOfTime2 = calculator.equationOfTime(JulianDay.getValue(of.minus(floor2, (long) TimeUnit.SECONDS).minus((int) ((equationOfTime - floor2) * 1.0E9d), (long) TimeUnit.NANOSECONDS), TimeScale.TT));
        long floor3 = (long) Math.floor(equationOfTime2);
        return of.minus(floor3, (long) TimeUnit.SECONDS).minus((int) ((equationOfTime2 - floor3) * 1.0E9d), (long) TimeUnit.NANOSECONDS);
    }
