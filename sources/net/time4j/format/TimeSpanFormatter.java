...
public abstract class TimeSpanFormatter<U, S extends TimeSpan<U>> {
        ...
        while (i2 < length) {
            ...
            if (charAt == '#') {
                ...
                if (charAt == '\'') {
                    ...
                    lastOn(arrayList).add(new SeparatorItem((char) ClassUtils.PACKAGE_SEPARATOR_CHAR, ','));
                } else if (charAt == ',') {
                    lastOn(arrayList).add(new SeparatorItem(',', (char) ClassUtils.PACKAGE_SEPARATOR_CHAR));
                } else if (charAt == '-') {
    ...
    public S parse(CharSequence charSequence, int i) throws ParseException {
        ...
        Map<U, Long> hashMap2 = new HashMap<>();
    ...
    public static class NumberItem<U> extends FormatItem<U> {
        ...
        */
        ...
        long getAmount(TimeSpan<? super U> timeSpan) {
            return timeSpan.getPartialAmount((U) this.unit);
        }
    ...
    public static class FractionItem<U> extends FormatItem<U> {
        ...
        @Override // net.time4j.format.TimeSpanFormatter.FormatItem
        void print(TimeSpan<? super U> timeSpan, Appendable appendable) throws IOException {
            String valueOf = String.valueOf(timeSpan.getPartialAmount((U) this.nanosecond));
        ...
        */
        ...
        @Override // net.time4j.format.TimeSpanFormatter.FormatItem
        boolean isZero(TimeSpan<? super U> timeSpan) {
            return timeSpan.getPartialAmount((U) this.nanosecond) == 0;
        }
    }
    ...
    public static class OptionalSectionItem<U> extends FormatItem<U> {
                ...
                if (parse < 0) {
                    while (true) {
                        ...
                        if (i2 >= size) {
                            i2 = -1;
                            break;
                        } else if (this.items.get(i2) == OrItem.INSTANCE) {
                            break;
                        }
                    }
                    if (i2 == -1) {
        ...
        @Override // net.time4j.format.TimeSpanFormatter.FormatItem
        boolean isZero(TimeSpan<? super U> timeSpan) {
            for (FormatItem<U> formatItem : this.items) {
                if (!formatItem.isZero(timeSpan)) {
                    return false;
                }
            }
            return true;
