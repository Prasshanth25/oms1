...
public final class CustomizedProcessor<V> implements FormatProcessor<V> {
    ...
    public FormatProcessor<V> quickPath(ChronoFormatter<?> chronoFormatter, AttributeQuery attributeQuery, int i) {
        ...
        boolean z;
        ChronoFormatter chronoFormatter3;
        boolean z2;
        ...
        if (chronoPrinter2 instanceof ChronoFormatter) {
            ChronoFormatter chronoFormatter4 = (ChronoFormatter) ChronoFormatter.class.cast(chronoPrinter2);
            chronoFormatter2 = chronoFormatter4.with(adjust(defaults, chronoFormatter4), attributeSet);
            z = true;
        } else {
            chronoFormatter2 = chronoPrinter;
            z = false;
        }
        ChronoParser<V> chronoParser2 = this.parser;
        if (chronoParser2 instanceof ChronoFormatter) {
            ChronoFormatter chronoFormatter5 = (ChronoFormatter) ChronoFormatter.class.cast(chronoParser2);
            chronoFormatter3 = chronoFormatter5.with(adjust(defaults, chronoFormatter5), attributeSet);
            z2 = true;
        } else {
            chronoFormatter3 = chronoParser;
            z2 = false;
        }
        return new CustomizedProcessor(this.element, chronoFormatter2, chronoFormatter3, z, z2, z3);
    }
